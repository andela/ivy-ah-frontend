import React from 'react';
import PropTypes from 'prop-types';
import { Formik } from 'formik';

import { Form, Input } from 'semantic-ui-react';
import Button from './Button';
import AuthErrorMessage from './AuthErrorMessage';
import { ResetPasswordSchema } from '../helpers/passwordValidation';

const ResetPasswordForm = ({
  authError, submit, loading, signedUp,
}) => (
  <Formik
    initialValues={{
      password: '',
      confirmPassword: '',
    }}
    validationSchema={ResetPasswordSchema}
    onSubmit={submit}
  >
    {({
      values,
      errors,
      touched,
      handleChange,
      handleBlur,
      handleSubmit,
    }) => {
      const formError = Object.keys(errors)
        .filter(keys => keys in touched)
        .map(key => errors[key]);
      return (
        <div
          className="yellow-box"
          style={{
            position: 'absolute',
            height: 'auth',
            width: '600px',
            margin: '-100px 0 0 -150px',
            top: '30%',
            left: '40%',
            padding: '35px',
            boxShadow: '2px 2px 12px 2px rgba(.2, .2, .2, .2)'
          }}
        >
          <div>
            <h2 style={{ textAlign: 'center', padding: '1rem', color: 'black' }}> Change your password</h2>
            <p style={{ textAlign: 'center', padding: '1rem', color: 'black' }}>
            Enter your new account password, minimum of 8 characters
            </p>
            <AuthErrorMessage
              hidden={!authError && formError.length === 0}
              error={authError || formError}
            />
            <Form onSubmit={handleSubmit} size="big">
              <Form.Field className="formInput">
                <Input
                  required
                  onBlur={handleBlur}
                  value={values.password}
                  name="password"
                  onChange={handleChange}
                  type="password"
                  control="input"
                  placeholder="Password"
                />
              </Form.Field>
              <Form.Field className="formInput">
                <Input
                  required
                  onBlur={handleBlur}
                  value={values.confirmPassword}
                  name="confirmPassword"
                  onChange={handleChange}
                  type="password"
                  control="input"
                  placeholder="Confirm Password"
                />
              </Form.Field>
              <Form.Field>
                <Button loading={loading} disabled={loading} type="blueButton">
                  {signedUp ? 'Password reset is successful' : 'Submit'}
                </Button>
              </Form.Field>
            </Form>
          </div>
        </div>
      );
    }}
  </Formik>
);

ResetPasswordForm.propTypes = {
  submit: PropTypes.func,
  signedUp: PropTypes.bool,
  loading: PropTypes.bool,
  authError: PropTypes.oneOf(['null', null, PropTypes.object]).isRequired,
};

const defaultFunc = input => input;

ResetPasswordForm.defaultProps = {
  signedUp: false,
  loading: false,
  submit: defaultFunc
};


export default ResetPasswordForm;
